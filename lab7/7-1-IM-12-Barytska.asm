...
public myPersonalA, four, one
extern myDenominatorCalc:PROTO
...
secondPartOfNum proc
    ...
    fld qword ptr [ecx+esi*8]            ;; 2c
    fld two
    fmul

    fld qword ptr [eax+esi*8]            ;; 2c - d
    fsub

    fld qword ptr [ebx]                  ;; 2c - d + root
    fadd
    fstp [numerator]
...
.while esi < 6
    ...
    mov eax, offset twenty_three
    mov ebx, offset myPersonalB
    call firstPartOfNum

    push offset myPersonalC
    push offset myPersonalD
    push offset root
    call secondPartOfNum
    
    call myDenominatorCalc
    fstp [denominator]
